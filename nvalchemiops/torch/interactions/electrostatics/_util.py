...
class _InjectCachedEvalGradWithFallback(torch.autograd.Function):
    ...
    inside backward and differentiates that true energy graph. A caller may
    supply a final tensor argument when its fallback must preserve an additional
    public differentiable input.
    """
    ...
    def setup_context(ctx, inputs, output):
        ...
        extra_tensor = inputs[13] if len(inputs) > 13 else None
        ctx.has_extra_tensor = isinstance(extra_tensor, torch.Tensor)
        saved_tensors = (
            positions,
            charges,
            cell,
        ...
        if ctx.has_extra_tensor:
            saved_tensors = (*saved_tensors, extra_tensor)
    ...
    def backward(ctx, grad_energy):
        ...
        extra_tensor = saved_tensors[6] if ctx.has_extra_tensor else None
        extra_for_grad = (
            extra_tensor.detach().requires_grad_(extra_tensor.requires_grad)
            if extra_tensor is not None
            else None
        )
        num_atoms = ctx.num_atoms

        def _pad(result, extra_grad=None):
            if ctx.has_extra_tensor:
                result = result + (None,) * (13 - len(result)) + (extra_grad,)
            return result + (None,) * (ctx.num_inputs - len(result))

        def _fallback(p, q, c, extra=None):
            if extra_for_grad is None:
                return ctx.fallback_fn(p, q, c)
            return ctx.fallback_fn(
                p,
                q,
                c,
                extra_for_grad if extra is None else extra,
            )

        def _extra_gradient():
            if extra_tensor is None or not extra_tensor.requires_grad:
                return None
            with torch.enable_grad():
                recomputed = _public_fallback_energy(
                    _fallback(positions, charges, cell, extra_tensor)
                )
        ...
        if use_fallback:
            ...
            if _has_potentially_geometry_dependent_charges(positions, charges):
                if create_graph:
                    ...
                    with torch.enable_grad():
                        recomputed = _public_fallback_energy(
                            _fallback(positions, charges, cell)
                        )
                        if diff_inputs:
