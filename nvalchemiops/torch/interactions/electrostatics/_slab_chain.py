...
__all__ = ["slab_correction_energy"]
...
def _slab_double_backward_launch(
    ...
    eps = 1.0e-5 if positions.dtype == torch.float64 else 1.0e-3
    pos_step = h_pos.to(dtype=positions.dtype)
    q_step = h_charge.to(dtype=charges.dtype)
    cell_step = h_cell.to(dtype=cell.dtype)

    plus = _slab_backward_values(
        positions + eps * pos_step,
        charges + eps * q_step,
        cell + eps * cell_step,
        pbc,
        batch_idx,
    ...
    scale = 1.0 / (2.0 * eps)
    grad_positions = (plus[0] - minus[0]) * scale
    grad_charges = (plus[1] - minus[1]) * scale
    grad_cell = (plus[2] - minus[2]) * scale
...
slab_correction_energy = _slab_chain["forward"]
