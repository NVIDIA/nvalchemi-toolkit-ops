...
def cluster_tile_neighbor_list(
    ...
    *,
    ...
    For ``format == "matrix"``:
        ...
        persist tile state for the next selective rebuild (unlike the Torch
        binding, which returns only the matrix triple or dual-cutoff matrix
        sextuple).
    For ``format == "coo"``:
        ...
        by ``(pair_energies, pair_forces)`` when ``pair_fn`` is set. For
        ``N == 0``, the early-return path emits the compact three-array tuple
        even when ``rebuild_flags`` is set.
    For ``format == "tile"``:
