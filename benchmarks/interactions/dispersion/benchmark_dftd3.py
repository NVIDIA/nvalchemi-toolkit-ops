...
from benchmarks.utils import (
    build_result, clean_gpu, create_run_directory, cuda_timed_runs, format_num,
    get_gpu_memory_info, get_gpu_sku, get_timestamp, make_csv_name, save_results,
)
from benchmarks.systems import (
    create_system, get_constant_atoms_configs,
    get_constant_total_configs, get_system_size_configs,
)
...
def benchmark_d3(data, cutoff, d3_params, d3_func_params, num_runs,
                 timing_mode='batch', warmup_runs=3):
    ...
    nbmat, _, nbmat_shifts = batch_cell_list(
        positions=pos_bohr, cell=cell_bohr, pbc=pbc,
        cutoff=cutoff_bohr, batch_idx=batch_idx, max_neighbors=maxnb,
    )
    dftd3(
        positions=pos_bohr, cell=cell_bohr, numbers=numbers,
        batch_idx=batch_idx,
        neighbor_matrix=nbmat, neighbor_matrix_shifts=nbmat_shifts,
        d3_params=d3_params, a1=a1, a2=a2, s8=s8,
    )
    ...
    time_nl = cuda_timed_runs(run_nl, num_runs, mode=timing_mode, warmup_runs=warmup_runs)
    ...
    time_d3 = cuda_timed_runs(run_d3, num_runs, mode=timing_mode, warmup_runs=warmup_runs)
...
def run_from_config(config, output_dir=None):
    ...
    d3_params_path = Path(config.get('params_path', '~/.cache/nvalchemiops/dftd3_parameters.pt'))
            ...
            for cfg in configs:
                ...
                try:
                    data = create_system(sys_name, num_atoms=n,
                                         pdb_path=cfg.get('pdb_path'), batch_size=bs)
                ...
                try:
                    ...
                    for cutoff in cutoffs:
                        ...
                        try:
                            r = benchmark_d3(
                                data, cutoff, d3_params, d3_func_params,
                                num_runs, timing_mode, warmup_runs,
                            )
                            result = build_result(
                                system=sys_name, scaling_mode=mode_name, method='dftd3',
                                ...
                                time_nl_us_per_atom=(r['time_nl_seconds'] * 1e6) / actual_total,
                                time_d3_us_per_atom=(r['time_d3_seconds'] * 1e6) / actual_total,
                            )
...
def parse_args():
    parser = argparse.ArgumentParser(description='DFT-D3 Benchmark (2 systems × 3 modes)')
